...
def extract_all():
    ...
    print("> Extraindo eventos de tempo diarios...")
    rows = fetchall(cur, """
    ...
    rows = fetchall(cur, "SELECT id, name FROM teams")
    data["equipes"] = [{"id": r[0], "nome": r[1]} for r in rows]

    rows = fetchall(cur, "SELECT id, name, channel_type FROM inboxes")
    data["inboxes"] = [{"id": r[0], "nome": r[1], "tipo": r[2]} for r in rows]

    rows = fetchall(cur, "SELECT id, name, email FROM users ORDER BY id")
...
def main():
    ...
    print("\n[INFO] Execute dashboard.html no navegador para visualizar.")
